import {ICookieService, ICookieServiceOption} from "@modern-app/shared/utils/interfaces";
...
export class BaseCookieService implements ICookieService {
  ...
  check(name: string): boolean {
    ...
    return regExp.test(<string>this.getCookieString());
  }
  ...
  get(name: string): string | null {
    if (this.check(name)) {
      name = encodeURIComponent(name);
      const regExp: RegExp = this.getCookieRegExp(name);
      const result: RegExpExecArray | null = regExp.exec(<string>this.getCookieString());
